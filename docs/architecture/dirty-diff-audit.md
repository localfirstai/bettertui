...
5. **Scheduler:** `scheduler.begin_frame()` → marks frame timing (***called AFTER paint***)
...
## Comparison: OpenTUI vs BetterTUI

### Diff Strategy

| Aspect | OpenTUI | BetterTUI | Impact |
|--------|---------|-----------|--------|
| Scan type | Full-screen cell scan | Full-screen → region merge | OpenTUI omits region merge step |
...
| Lazy frames | Only emits if cells changed | Always emits | OpenTUI produces 0 bytes for unchanged frames |
| Run-length | Yes (style runs coalesced) | Per-cell SGR | OpenTUI significantly less ANSI bytes |
| Cursor diff | Yes (cached state) | No | OpenTUI avoids redundant cursor sequences |

### Performance Considerations

1. **OpenTUI's approach scales with screen size, not change count:** It scans ALL cells every frame but uses SoA layout (4 parallel arrays) for cache-friendly access.
...
3. **Run-length encoding in OpenTUI** reduces ANSI output significantly for same-styled runs.
4. **No-op frames in OpenTUI** emit 0 bytes vs BetterTUI always emits hide/show cursor.

### Key Advantage: OpenTUI
...
| 12 | OpenTUI comparison report | ✅ | Embedded in audit |

## Remaining Gaps (Post-v1)

1. **Frame suppression:** Emit 0 bytes for identical frames (OpenTUI does this)
