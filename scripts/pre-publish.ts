 ...
 * - Versions match across packages
 * - Dist directories exist
 * - Native packages exist and are valid
...
const ALL_PACKAGES: PackageConfig[] = [
  ...
  },
  {
    ...
    rootDir: join(rootDir, "packages", "shared"),
    distDir: join(rootDir, "packages", "shared", "dist"),
  },
];
...
function validatePackage(config: PackageConfig): void {
  ...
  if (!existsSync(distPackageJsonPath)) {
    ...
    console.error("Please rebuild the package with 'pnpm build'");
    process.exit(1);
  }
  ...
  // For packages that publish a @bettertui/core dependency, check the version
  if (config.requiresCore) {
    const coreDependencyVersion = distPackageJson.dependencies?.["@bettertui/core"];
    if (coreDependencyVersion !== packageJson.version) {
      console.error("ERROR: @bettertui/core dependency version mismatch in dist");
      console.error(`  Expected: ${packageJson.version}`);
      console.error(`  Found: ${coreDependencyVersion}`);
      console.error("Please rebuild the package with 'pnpm build'");
      process.exit(1);
    }
    console.log("SUCCESS: @bettertui/core dependency version matches");
  }
...
2. [OK] All packages have been built (pnpm build)
...
function main(): void {
  console.log("BetterTUI Pre-Publish Validation");
  ...
  console.log("\nNext steps:");
  console.log("  • Run: node scripts/publish.ts");
}
