# Counter

> A keyboard-driven counter showing state, badges, and a status line.

- **Category:** Getting Started
- **Level:** 1 / 5
- **Demonstrates:** useKeyboard, Badge, StatusLine, state
- **Requires:** _None._

## What it shows

This example focuses on **useKeyboard**. Read the source in
`src/counter.tsx` — each example is small, self-contained, and commented.

## Run it

```bash
pnpm --filter @bettertui/examples build
node dist/index.mjs counter
```

Or from the example browser:

```bash
pnpm --filter @bettertui/examples dev
```

## Key APIs

- `useKeyboard`
- `Badge`
- `StatusLine`
- `state`

## Common mistakes

- Forgetting to call `runtime?.runtime.dispose()` before `process.exit(0)` on quit.
- Mutating state without re-rendering — call `render(<App />)` after changes.
- Assuming a mouse/PTY capability is present; check `requires` above first.

## Next examples

- `form-controls` — Form Controls
- `data-table-basics` — Data Tables
