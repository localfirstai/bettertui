...
import * as Filters from "@bettertui/core";
import { ThreeCliRenderer } from "@bettertui/core";
import * as Matrices from "@bettertui/core";
import {
  AmbientLight,
  BoxGeometry,
  Color,
...
interface ShaderCubeDemoState {
  ...
  filterFunctions: {
    name: string;
    func: ((buffer: OptimizedBuffer, deltaTime: number) => void) | null;
  }[];
...
export async function run(renderer: CliRenderer): Promise<void> {
  ...
  class ColorMatrixEffect {
    ...
    public previousMatrix(): void {
      this.currentIndex =
        (this.currentIndex - 1 + colorMatrixRegistry.length) % colorMatrixRegistry.length;
    }
  }
  ...
  const filterFunctions: {
    ...
    {
      name: "CRT Rolling Bar",
      func: crtRollingBarEffectInstance.apply.bind(crtRollingBarEffectInstance),
    },
    {
      name: "Pipboy",
      func: (buf, dt) => {
    ...
    { name: "Gain", func: (buf, _dt) => Filters.applyGain(buf, gainValue) },
    {
      name: "Saturation",
      func: (buf, _dt) =>
        Filters.applySaturation(buf, saturationCellMask ?? undefined, saturationValue),
    },
  ];
  ...
  const emissiveMapTexture = TextureUtils.createGradient(
    256,
    new Color(1, 0.6, 0),
    new Color(0, 0, 0),
    "radial",
  ...
  const materials: MeshPhongMaterial[] = [
    ...
    new MeshPhongMaterial({
      map: yellowTexture,
      shininess: 30,
      specular: new Color(0.8, 0.8, 0.8),
    }),
  ...
  const keyHandler = (key: KeyEvent) => {
    ...
    if (key.name === "j") {
      currentFilterIndex =
        (currentFilterIndex - 1 + filterFunctions.length) % filterFunctions.length;
      filterChanged = true;
    } else if (key.name === "k") {
    ...
    if (key.name === "t" && filterFunctions[currentFilterIndex].name === "Saturation") {
      ...
      if (saturationFullScreen) {
        ...
        saturationCellMask = createRightHalfCellMask(
    ...
    if (key.name === "[") {
      switch (currentFilterName) {
        ...
        case "Rainbow Text":
          ...
          break;
        case "CRT Rolling Bar":
          crtRollingBarEffectInstance.speed = Math.max(
            0.1,
            crtRollingBarEffectInstance.speed - 0.1,
          );
          paramChanged = true;
          break;
        case "Pipboy":
      ...
      switch (currentFilterName) {
        ...
        case "Rainbow Text":
          ...
          break;
        case "CRT Rolling Bar":
          crtRollingBarEffectInstance.speed = Math.min(
            5.0,
            crtRollingBarEffectInstance.speed + 0.1,
          );
          paramChanged = true;
    ...
    if (key.name === "{") {
      switch (currentFilterName) {
        case "Distortion":
          distortionEffectInstance.maxGlitchLines = Math.max(
            0,
            distortionEffectInstance.maxGlitchLines - 1,
          );
          paramChanged = true;
          break;
        case "Clouds":
        ...
        case "Flames":
          ...
          break;
        case "Rainbow Text":
          rainbowTextEffectInstance.repeats = Math.max(
            1.0,
            rainbowTextEffectInstance.repeats - 0.5,
          );
          paramChanged = true;
          break;
        case "CRT Rolling Bar":
          crtRollingBarEffectInstance.intensity = Math.max(
            0.0,
            crtRollingBarEffectInstance.intensity - 0.05,
          );
          paramChanged = true;
          break;
        case "Pipboy":
          pipboyVignetteEffectInstance.strength = Math.max(
            0.0,
            pipboyVignetteEffectInstance.strength - 0.05,
          );
          paramChanged = true;
          break;
      }
      ...
      switch (currentFilterName) {
        case "Distortion":
          distortionEffectInstance.maxGlitchLines = Math.min(
            height - 1,
            distortionEffectInstance.maxGlitchLines + 1,
          );
          paramChanged = true;
        ...
        case "Flames":
          ...
          break;
        case "Rainbow Text":
          rainbowTextEffectInstance.repeats = Math.min(
            20.0,
            rainbowTextEffectInstance.repeats + 0.5,
          );
          paramChanged = true;
          break;
        case "CRT Rolling Bar":
          crtRollingBarEffectInstance.intensity = Math.min(
            1.0,
            crtRollingBarEffectInstance.intensity + 0.05,
          );
          paramChanged = true;
          break;
        case "Pipboy":
          pipboyVignetteEffectInstance.strength = Math.min(
            3.0,
            pipboyVignetteEffectInstance.strength + 0.05,
          );
          paramChanged = true;
