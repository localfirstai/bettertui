...
export async function run(renderer: CliRenderer): Promise<void> {
  ...
  const charDelay = 0.1;
  const charRandomOffsets: number[] = [...opentuiChars, ...fiveKChars].map(
  ...
  renderer.setFrameCallback(async (deltaMs) => {
    ...
    if (starObject) {
      ...
      particleSystem.setEmitterPosition(
        starObject.position.x,
        starObject.position.y,
        starObject.position.z,
      );
