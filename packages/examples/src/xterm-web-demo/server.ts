...
function drawPaddle(
  ...
  const top = clamp(
    Math.round((centerY - PADDLE_HALF_HEIGHT) * 4),
    0,
    BOARD_PIXEL_HEIGHT - PADDLE_PIXEL_HEIGHT,
  );
...
function drawBallTrail(maskBuffer: Uint8Array, styleBuffer: Uint8Array) {
  for (let age = ballTrail.length - 1; age >= 0; age -= 1) {
    ...
    const style =
      age <= 1 ? BOARD_STYLE_TRAIL_1 : age <= 3 ? BOARD_STYLE_TRAIL_2 : BOARD_STYLE_TRAIL_3;
...
function scaleBoardBuffers(
  sourceMask: Uint8Array,
  sourceStyle: Uint8Array,
  boardWidth: number,
  boardHeight: number,
) {
  ...
  for (let y = 0; y < targetPixelHeight; y += 1) {
    const sourceY = Math.min(
      ...
      Math.floor((y * BOARD_PIXEL_HEIGHT) / targetPixelHeight),
    );
    ...
    for (let x = 0; x < targetPixelWidth; x += 1) {
      const sourceX = Math.min(
        BOARD_PIXEL_WIDTH - 1,
        Math.floor((x * BOARD_PIXEL_WIDTH) / targetPixelWidth),
      );
...
function buildBoardFrame(boardWidth: number, boardHeight: number) {
  ...
  const { maskBuffer, styleBuffer } = scaleBoardBuffers(
    source.maskBuffer,
    source.styleBuffer,
    boardWidth,
    boardHeight,
