...
const AUTO_EXIT_MS =
  Number.isFinite(AUTO_EXIT_ENV) && AUTO_EXIT_ENV > 0 ? AUTO_EXIT_ENV : DEFAULT_AUTO_EXIT_MS;
...
function buildSimpleTextBoxSnapshot(
...
class SplitFooterDemo {
  ...
  private publishMessage(role: MessageRole, text: string, countAsMessage = true): void {
    if (countAsMessage) {
      this.messageCount += 1;
    }
  ...
  private setStreamInterval(requestedInterval: number): void {
    const clampedInterval = Math.min(
      Math.max(requestedInterval, MIN_STREAM_INTERVAL),
      MAX_STREAM_INTERVAL,
    );
    ...
    switch (command.toLowerCase()) {
      ...
      case "/speed": {
        if (args.length === 0) {
          this.publishMessage(
            "system",
            `Current stream interval is ${this.streamIntervalMs}ms.`,
            false,
          );
          return;
        }
