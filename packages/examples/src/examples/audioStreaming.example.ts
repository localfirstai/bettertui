...
class AudioStreamingDemo {
  ...
  private toggleGroup(): void {
    ...
    this.statusMessage = nextDimmed
      ? "Stream routed through 35% group"
      : "Stream routed through full group";
    this.statusColor = PALETTE.accent;
    this.refreshText();
  }
  ...
  private computeSpectrum(pcm: Float32Array): void {
    ...
    for (let band = 0; band < BAND_CENTERS.length; band += 1) {
      ...
      const low = previous
        ? Math.sqrt(previous * center)
        : center / Math.sqrt((next ?? center * 2) / center);
      const high = next
