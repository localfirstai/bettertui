...
let selectedPlaybackChannelIndex = Math.max(
  0,
  PLAYBACK_CHANNEL_OPTIONS.indexOf(DEFAULT_PLAYBACK_CHANNELS),
);
let activePlaybackChannels =
  PLAYBACK_CHANNEL_OPTIONS[selectedPlaybackChannelIndex] ?? DEFAULT_PLAYBACK_CHANNELS;
...
function selectMixTarget(step: number): void {
  selectedMixTargetIndex =
    (selectedMixTargetIndex + step + MIX_TARGETS.length) % MIX_TARGETS.length;
...
function computeSpectrum(pcm: Float32Array, channels: number): string {
  ...
  for (let bucket = 0; bucket < FFT_BINS; bucket += 1) {
    ...
    const end = Math.min(
      nyquistBins,
      Math.max(start + 1, Math.ceil((hz1 / nyquistHz) * (nyquistBins - 1))),
    );
  ...
  for (let i = 0; i < VIS_BARS; i += 1) {
    ...
    fftVizBars[i] =
      incoming > previous
        ? incoming
        : previous * (1 - VIS_BAR_RELEASE) + incoming * VIS_BAR_RELEASE;
  }
  ...
  for (let i = 0; i < FFT_BANDS; i += 1) {
    ...
    const highBin = Math.min(
      FFT_BINS - 1,
      Math.max(lowBin, Math.ceil((high / nyquistHz) * (FFT_BINS - 1))),
    );
...
function truncateDeviceName(name: string, maxLength = 40): string {
...
function selectedPlaybackDevicePosition(): number {
  ...
  const position = playbackDevices.findIndex(
    (device) => device.index === selectedPlaybackDeviceIndex,
  );
...
async function classifyFilePickerEntry(
  ...
  if (isFile)
    return isSupportedAudioFileName(name) ? { type: "file", path: entryPath, name } : null;
...
async function initializeAudioForOutputConfig(
  ...
  if (playbackDevices.length > 0) {
    ...
    if (
      selectedPlaybackDeviceIndex != null &&
      nextAudio.selectPlaybackDevice(selectedPlaybackDeviceIndex)
    ) {
  ...
  if (!outputStarted) {
    ...
    if (!outputStarted) {
      lastAction = nextAudio.startMixer()
        ? "Playback unavailable; mixer-only mode"
    ...
    lastAction = activeDevice
      ? `Output device ${truncateDeviceName(activeDevice.name, 26)}`
      : "Output started";
  } else {
    lastAction = "Output started with default device";
  }
  ...
  groups =
    sfxGroup != null && musicGroup != null && uiGroup != null
      ? { sfx: sfxGroup, music: musicGroup, ui: uiGroup }
      : null;
...
function stepSampleRate(step: number): void {
  ...
  selectedSampleRateIndex =
    (selectedSampleRateIndex + step + SAMPLE_RATE_OPTIONS.length) % SAMPLE_RATE_OPTIONS.length;
...
function stepPlaybackChannels(step: number): void {
  ...
  selectedPlaybackChannelIndex =
    (selectedPlaybackChannelIndex + step + PLAYBACK_CHANNEL_OPTIONS.length) %
    PLAYBACK_CHANNEL_OPTIONS.length;
...
function refreshPlaybackDevices(keepSelection = true): boolean {
  ...
  activePlaybackDeviceIndex =
    previousActive != null && playbackDevices.some((device) => device.index === previousActive)
      ? previousActive
      : null;
...
function updateAudioView(deltaMs = 16): void {
...
export async function run(renderer: CliRenderer): Promise<void> {
  ...
  selectedPlaybackChannelIndex = Math.max(
    0,
    PLAYBACK_CHANNEL_OPTIONS.indexOf(activePlaybackChannels),
  );
