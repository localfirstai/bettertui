...
fn run_scenario(scenario: &str) -> Vec<u8> {
    ...
    let config = PtyConfig::new(binary.to_str().unwrap())
        .with_args(vec![scenario.to_string()])
...
fn e2e_layout_basic_text() {
    ...
    assert!(
        contents.contains("Hello Layout E2E"),
        "basic: should contain text, got:\n{contents}"
    );
    assert!(
        contents.starts_with("Hello Layout E2E"),
        "basic: text should start at row 0, got:\n{contents}"
    );
}
...
fn e2e_layout_flex_row() {
    ...
    let contents = screen.contents();
    assert!(
        contents.contains("Left"),
        "flex-row: should contain Left, got:\n{contents}"
    );
    assert!(
        contents.contains("Right"),
        "flex-row: should contain Right, got:\n{contents}"
    );
...
fn e2e_layout_flex_column() {
    ...
    assert!(
        all.len() > 1,
        "flex-column: expected at least 2 rows, got {}",
        all.len()
    );
    assert!(
        all[0].contains("Top"),
        "flex-column: row 0 should contain Top, got:\n{}",
        all[0]
    );
    assert!(
        all[1].contains("Bottom"),
        "flex-column: row 1 should contain Bottom, got:\n{}",
        all[1]
    );
}
...
fn e2e_layout_styled_text() {
    ...
    assert!(
        ...
        "styled: should contain Red, got:\n{contents}"
    );
    assert!(
        contents.contains("Green"),
        "styled: should contain Green, got:\n{contents}"
    );
    assert!(
        contents.contains("Blue"),
        "styled: should contain Blue, got:\n{contents}"
    );
...
fn e2e_layout_nested_padding() {
    ...
    assert!(
        all.len() > 2,
        "nested: expected at least 3 rows, got {}",
        all.len()
    );
    ...
    if let Some(cell) = screen.cell(2, 5) {
        assert_eq!(
            cell.contents(),
            "I",
            "nested: cell at (2,5) should be 'I', got {:?}",
            cell.contents()
        );
    }
}
