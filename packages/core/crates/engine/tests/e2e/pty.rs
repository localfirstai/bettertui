...
fn e2e_pty_spawn_echo_raw() {
    ...
    assert!(
        ...
        "raw output should contain echo text"
    );
}
...
fn e2e_pty_ansi_color_parsed_with_vt100() {
    ...
    let mut process = PtyProcess::spawn(
        PtyConfig::new("printf")
            .with_args(vec![
                "\\033[31mRed\\033[0m \\033[32mGreen\\033[0m".to_string(),
            ])
            .with_size(PtySize::new(80, 24)),
    )
    ...
    assert!(
        ...
        "vt100 should parse ANSI green text"
    );
    ...
    if let Some(cell) = screen.cell(0, green_col) {
        assert_eq!(
            cell.fgcolor(),
            vt100::Color::Idx(2),
            "Green text should have green foreground"
        );
    }
}
...
fn e2e_pty_process_lifecycle() {
    let mut process = PtyProcess::spawn(PtyConfig::new("true").with_size(PtySize::new(80, 24)))
