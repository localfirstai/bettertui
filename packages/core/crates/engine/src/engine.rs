...
impl Engine {
    ...
    pub fn new() -> Self {
        ...
        Self { processor: CommandProcessor::new(), frame_count: 0 }
    }
    ...
    pub fn process_commands(&mut self, commands: Vec<Command>) -> CommandResult {
        debug!(command_count = commands.len(), "Engine::process_commands() - processing batch");
        self.processor.process_batch(commands)
    }

    /// Processes a single command.
    pub fn process_command(&mut self, command: Command) -> Result<(), crate::protocol::CommandError> {
        debug!(?command, "Engine::process_command() - processing single command");
    ...
    pub fn begin_frame(&mut self) {
        self.frame_count += 1;
        debug!(frame_id = self.frame_count, "Engine::begin_frame() - starting frame");
        let _ = self.processor.process_single(Command::BeginFrame { frame_id: self.frame_count });
    }

    /// Commits the current frame.
    pub fn commit_frame(&mut self) {
        debug!(frame_id = self.frame_count, "Engine::commit_frame() - committing frame");
        let _ = self.processor.process_single(Command::CommitFrame { frame_id: self.frame_count });
    }
    ...
    pub fn append_child(&mut self, parent: NodeId, child: NodeId) -> Result<(), crate::protocol::CommandError> {
    ...
    pub fn set_text(&mut self, id: NodeId, text: impl Into<String>) {
        let _ = self.processor.process_single(Command::SetText { id, text: text.into() });
    }

    /// Sets the style on a node.
    pub fn set_style(&mut self, id: NodeId, style: crate::tree::Style) {
        let _ = self.processor.process_single(Command::SetStyle { id, style });
    }

    /// Sets the layout on a node.
    pub fn set_layout(&mut self, id: NodeId, layout: crate::taffy::LayoutProps) {
        let _ = self.processor.process_single(Command::SetLayout { id, layout });
    }
    ...
    pub fn tree_summary(&self) -> String {
        ...
        format!("Tree Summary:\n  Nodes: {}\n  Frames: {}\n  Generation: {}", node_count, frame_count, generation)
    }
}
...
impl Inspector {
    ...
    pub fn new() -> Self {
        Self { command_log: Vec::new(), mutation_log: Vec::new() }
    }
    ...
    pub fn log_mutation(&mut self, node_id: NodeId, mutation_type: MutationType, timestamp: u64) {
        self.mutation_log.push(MutationEntry { node_id, mutation_type, timestamp });
    }
    ...
    fn print_node_detail(&self, arena: &NodeArena, id: NodeId, output: &mut String, prefix: &str, is_last: bool) {
    ...
    pub fn tree_summary(&self, arena: &NodeArena) -> TreeSummary {
        ...
        TreeSummary { total_nodes, max_depth, kind_counts }
    }

    /// Returns the last N commands from the log.
    pub fn recent_commands(&self, n: usize) -> Vec<&Command> {
        self.command_log.iter().rev().take(n).map(|entry| &entry.command).collect()
    }
...
mod tests {
    ...
    fn engine_process_command() {
        let mut engine = Engine::new();
        let id = engine.create_node(NodeKind::Text);
        let result = engine.process_command(crate::protocol::Command::SetText { id, text: "from command".into() });
        assert!(result.is_ok());
        let node = engine.get_node(id).unwrap();
    ...
    fn engine_process_commands_batch() {
        ...
        let cmd1 = Command::SetText { id, text: "batch1".into() };
        let cmd2 = Command::SetText { id, text: "batch2".into() };
    ...
    fn inspector_clear() {
        ...
        insp.log_mutation(crate::tree::NodeArena::new().root(), MutationType::Created, 0);
        insp.clear();
        assert!(insp.command_log().is_empty());
        assert!(insp.mutation_log().is_empty());
    }
    ...
    fn inspector_commands_for_node() {
        ...
        let mut insp = Inspector::new();
        let id = crate::tree::NodeArena::new().root();
        insp.log_command(Command::SetText { id, text: "a".into() }, 1);
    ...
    fn command_entry_accessors() {
        ...
        let entry = CommandEntry { command: cmd, timestamp: 99 };
    ...
    fn mutation_entry_accessors() {
        let id = crate::tree::NodeArena::new().root();
        let entry = MutationEntry { node_id: id, mutation_type: MutationType::Modified, timestamp: 5 };
