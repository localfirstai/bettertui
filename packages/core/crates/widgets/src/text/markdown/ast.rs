...
pub enum MarkdownNode {
    ...
    CodeBlock { language: Option<Box<str>>, code: Box<str> },
    Table { headers: Vec<Vec<InlineNode>>, rows: Vec<Vec<Vec<InlineNode>>> },
    HorizontalRule,
...
impl InlineNode {
    pub fn text_content(&self) -> &str {
        match self {
            ...
            | InlineNode::Strikethrough(children) => children.first().map(|c| c.text_content()).unwrap_or(""),
...
mod tests {
    ...
    fn inline_node_link_text_content() {
        let node = InlineNode::Link { text: "click".into(), url: "https://example.com".into() };
        assert_eq!(node.text_content(), "click");
    }

    #[test]
    fn markdown_node_variants() {
        let heading = MarkdownNode::Heading { level: 1, content: vec![InlineNode::Text("Title".into())] };
    ...
    fn list_item_creation() {
        let item = ListItem { content: vec![InlineNode::Text("item".into())], children: vec![] };
    ...
    fn task_item_creation() {
        let item = TaskItem { checked: true, content: vec![InlineNode::Text("done".into())] };
        assert!(item.checked);
    }
}
