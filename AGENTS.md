...
- **`@bettertui/core` owns both TypeScript runtime and Rust engine:** The engine bridge (internal to core as `src/engine/`) imports `Command` and `CommandBuffer` from core. The Rust crates live in `packages/core/crates/` — `engine`, `widgets`, and `bindings`. Everything from the Rust engine and widgets crates is exposed through `@bettertui/core` only.
